...
if (items.length > 0){
    selectedItemIndexStore.set(items.length -1 ); 
}else {
...
async function checkHit(e, ctx) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const itemObject = ItemsMap.get(item.itemExtra.type);
        if (itemObject && itemObject.isHit(item.itemExtra,x, y, ctx)) {
...
{#if items}
    ...
    <div class="flex w-full p-0 m-0 bg-stone-900 text-white  gap-1">
        <div class='mx-1'>
            <CanvasPlayer
                {items}
                {slideExtra}
