...
static boundingRectangleX(itemExtra) {
  return Math.min(itemExtra.x1, itemExtra.x2);
}

static boundingRectangleY(itemExtra) {
  return Math.min(itemExtra.y1, itemExtra.y2);
}

static width(itemExtra) {
  return Math.abs(itemExtra.x2 - itemExtra.x1);
}

static height(itemExtra) {
  return Math.abs(itemExtra.y2 - itemExtra.y1);
}

static setWidth(itemExtra, newWidth) {
  itemExtra.x2 = itemExtra.x1 + newWidth;
}

static setHeight(itemExtra, newHeight) {
  itemExtra.y2 = itemExtra.y1 + newHeight;
}
