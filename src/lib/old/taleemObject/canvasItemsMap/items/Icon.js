...
//-------------text
  ...
  static isHit(itemExtra, mouseX, mouseY) {
    const width = Icon.width(itemExtra);
    const height = Icon.height(itemExtra);
    return (
      mouseX >= itemExtra.x &&
      mouseX <= itemExtra.x + width &&
      mouseY >= itemExtra.y &&
      mouseY <= itemExtra.y + height
    );
  }
  ...
  static width(itemExtra) {
    const percent_rect_extra = 20;
    const textWidth = itemExtra.fontSize * itemExtra.text.length * 0.6;
    const iconWidth = itemExtra.iconSize;
    const largerWidth = Math.max(textWidth, iconWidth);
    return largerWidth + (percent_rect_extra * (largerWidth / 100));
  }

  static height(itemExtra) {
    return itemExtra.fontSize + itemExtra.iconSize;
  }

  static setWidth(itemExtra, newWidth) {
    itemExtra.fontSize += newWidth / 10; // Scale font size
  }

  static setHeight(itemExtra, newHeight) {
    itemExtra.iconSize += newHeight / 10; // Scale icon size
  }
