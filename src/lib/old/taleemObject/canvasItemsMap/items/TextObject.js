...
export default class TextObject {
  ...
  static isHit(itemExtra, mouseX, mouseY) {
    const width = TextObject.width(itemExtra);
    const height = TextObject.height(itemExtra);
    return (
      mouseX >= itemExtra.x &&
  ...
  static width(itemExtra) {
    return itemExtra.fontSize * itemExtra.text.length * 0.6;
  }

  static height(itemExtra) {
    return itemExtra.fontSize;
  }
