...
///////////////////////////////////////////////////
  ...
  static isHit(itemExtra, mouseX, mouseY) {
    const width = List.width(itemExtra);
    const height = List.height(itemExtra);
    return (
      mouseX >= itemExtra.x &&
  ...
  static width(itemExtra) {
    const lines = itemExtra.text.split("\n");
    return Math.max(...lines.map(line => line.length * (itemExtra.fontSize / 2)));
  }

  static height(itemExtra) {
    const lines = itemExtra.text.split("\n").length;
    return lines * (itemExtra.fontSize + itemExtra.lineHeightOffset);
  }

  static setWidth(itemExtra, newWidth) {
    itemExtra.fontSize = newWidth / 10;
  }

  static setHeight(itemExtra, newHeight) {
    itemExtra.fontSize = newHeight / 10;
  }
