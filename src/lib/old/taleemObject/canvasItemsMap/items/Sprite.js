...
/////////////////////////////////////////////////////////////////                                      
  ...
  static isHit(itemExtra, mouseX, mouseY) {
    return (
      mouseX >= itemExtra.x &&
      mouseX <= itemExtra.x + Math.abs(itemExtra.width) * 100 &&
      mouseY >= itemExtra.y &&
      mouseY <= itemExtra.y + Math.abs(itemExtra.height) * 100
    );
  }
  ...
  static width(itemExtra) {
    return itemExtra.width * 100; // Assuming width is a scale factor
  }

  static height(itemExtra) {
    return itemExtra.height * 100; // Assuming height is a scale factor
  }

  static setWidth(itemExtra, newWidth) {
    itemExtra.width = newWidth / 100;
  }

  static setHeight(itemExtra, newHeight) {
    itemExtra.height = newHeight / 100;
  }
