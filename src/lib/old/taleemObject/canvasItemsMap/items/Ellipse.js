...
return dialogueBox;
  ...
  static isHit(itemExtra, mouseX, mouseY) {
    ...
    const cos = Math.cos(-itemExtra.rotation * (Math.PI / 180));
    const sin = Math.sin(-itemExtra.rotation * (Math.PI / 180));
    
    // Rotate the point back
    const localX = cos * dx - sin * dy;
    const localY = sin * dx + cos * dy;

    // Check ellipse equation (normalized hit test)
    return (
      (localX * localX) / (itemExtra.radiusX * itemExtra.radiusX) +
      (localY * localY) / (itemExtra.radiusY * itemExtra.radiusY)
      <= 1
    );
  }

  static boundingRectangleX(itemExtra) {
    return itemExtra.x - itemExtra.radiusX;
  }

  static boundingRectangleY(itemExtra) {
    return itemExtra.y - itemExtra.radiusY;
  }

  static width(itemExtra) {
    return itemExtra.radiusX * 2;
  }

  static height(itemExtra) {
    return itemExtra.radiusY * 2;
  }

  static setWidth(itemExtra, newWidth) {
    itemExtra.radiusX = newWidth / 2;
  }

  static setHeight(itemExtra, newHeight) {
    itemExtra.radiusY = newHeight / 2;
  }
